...
namespace SS
{
  ...
  public abstract class AbstractSpreadsheet
  {
    ...
    {
      // Adds the current cell name to the visited list.
      visited.Add( name );
      
      // For every direct dependent of the currrent cell ...
      foreach ( string n in GetDirectDependents( name ) )
      {
        if ( n.Equals( start ) )
        {
          // Throw Circular Exception if there is a circular dependency.
          throw new CircularException();
        }
        else if ( !visited.Contains( n ) )
        {
        // Otherwise if the cell's dependent has not been visited, call the visit method again
        // on this cell's current dependent.
          Visit( start, n, visited, changed );
        }
      }

      // Add the current cell to the changed linked list.
