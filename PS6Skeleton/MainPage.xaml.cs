...
public partial class MainPage : ContentPage
{
    ...
    private void displaySelection(SpreadsheetGrid grid)
    {
        ...
        // Updates Displayed Cell Name
        ChangeDisplayedCellName(col, row);

        // Updates Displayed Value
        cellValue.Text = "Value: " + value;

        // Updates Displayed Contents
        cellContents.Text = "";
    ...
    private void ChangeDisplayedCellName(int col, int row)
    {
        string alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        char[] alphaChars = alphabet.ToCharArray();
        cellName.Text = alphaChars[col].ToString() + (row + 1);
    }
